...
# 综合推荐评分权重（入门模式：不看相关性，只看热门度+质量，适合初学者系统性了解ML）
WEIGHTS_BEGINNER = {
    'recency': 0.10,
    'popularity': 0.60,
    'quality': 0.30,
...
def calculate_recommendation_score(
    ...
    权重定义在模块顶部常量中。
    - beginner 模式：WEIGHTS_BEGINNER（热门度50% + 质量30% + 新近性20%）
    - hot 论文：WEIGHTS_HOT
    - 普通模式：WEIGHTS_NORMAL
...
def filter_and_score_papers(
    ...
    Args:
        ...
        focus_keywords: 用户关注关键词（可选）
        mode: 评分模式 ('normal' | 'beginner')，入门模式不过滤相关性为0的论文
    ...
    for paper in papers:
        ...
        # 普通模式：相关性为0则跳过；入门模式：保留所有论文
        if relevance == 0 and mode != 'beginner':
            continue
        ...
        recommendation_score = calculate_recommendation_score(
            relevance, recency, popularity, quality, is_hot_paper_batch, mode=mode
        )
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='normal',
                        choices=['normal', 'beginner'],
                        help='Scoring mode: normal (relevance-based) or beginner (popularity+quality, no research interest filter)')
    ...
    mode = args.mode
    if mode == 'beginner':
        logger.info("Mode: beginner (popularity+quality scoring, no research interest filter)")
    if focus_keywords:
            ...
            if mode == 'beginner':
                ...
                for query in beginner_queries:
                    try:
                        ...
                        logger.warning("S2 query '%s' failed (non-fatal): %s", query, e)
                    time.sleep(S2_CATEGORY_REQUEST_INTERVAL)
                hot_papers.sort(key=lambda x: x.get("influentialCitationCount", 0), reverse=True)
                logger.info("Beginner mode: found %d hot papers across broad ML topics", len(hot_papers))
            else:
                try:
