...
At the start of execution, read the config file to detect the scoring mode:

```bash
# Read mode from config
MODE=$(grep -E "^\s*mode:" "$OBSIDIAN_VAULT_PATH/99_System/Config/research_interests.yaml" | awk '{print $2}' | tr -d '"')
...
**Mode types**:
- `normal` (default): Uses research interest relevance (40%) + popularity (30%) + recency (20%) + quality (10%)
- `beginner`: Uses popularity (50%) + quality (30%) + recency (20%) — no research interest filtering, ideal for beginners systematically learning ML
...
**入门模式 (beginner)**: 如果你是ML初学者，设置 `mode: beginner` 后，将不再按研究方向过滤论文，而是纯粹按热门度和质量评分，帮你系统性地了解机器学习领域的整体进展。
...
综合多个维度的评分。根据 config 中的 `mode` 设置，使用不同的权重：

**入门模式 (`mode: beginner`)**：适合ML初学者，不按研究方向过滤，只看热门度和质量。
...
1. **创建推荐笔记文件**
   - 文件名（根据语言设置）：
     - 中文（`language: "zh"`）：`5_Daily/YYYY-MM-DD论文推荐.md`
     - 英文（`language: "en"`）：`5_Daily/YYYY-MM-DD-paper-recommendations.md`
   - 使用变量：`5_Daily/YYYY-MM-DD${NOTE_SUFFIX}.md`（其中 `NOTE_SUFFIX` 在语言检测阶段已设置）
   - 必须包含属性：
...
python scripts/link_keywords.py \
  ...
  --input "5_Daily/YYYY-MM-DD${NOTE_SUFFIX}.md" \
  --output "5_Daily/YYYY-MM-DD${NOTE_SUFFIX}_linked.md"
```
...
- **文件名以日期**：保持 `5_Daily/YYYY-MM-DD${NOTE_SUFFIX}.md` 格式（中文：`论文推荐`，英文：`paper-recommendations`）
...
5. **生成推荐笔记（包含关键词链接）**
   - 创建 `5_Daily/YYYY-MM-DD${NOTE_SUFFIX}.md`（使用目标日期，`NOTE_SUFFIX` 依语言设置）
   - **按评分排序**：所有论文按推荐评分从高到低排列
   - **前3篇特殊处理**：
     - 论文名称用 wikilink 格式：`[[论文名字]]`
