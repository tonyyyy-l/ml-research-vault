...
The config file should be located at: `${OBSIDIAN_VAULT_PATH:-Research Vault}/99_System/Config/research_interests.yaml`
...
VAULT_ROOT="${OBSIDIAN_VAULT_PATH:-Research Vault}"
...
```bash
# 先在仓库根目录初始化路径，再创建工作目录
REPO_ROOT="${REPO_ROOT:-$(pwd)}"
VAULT_ROOT="${OBSIDIAN_VAULT_PATH:-${REPO_ROOT}/Research Vault}"
...
# 根据论文内容确定领域
# 推断规则：
# - 如果提到"agent/swarm/multi-agent/orchestration" → Agents
# - 如果提到"vision/visual/image/video" → Computer-Vision
# - 如果提到"reinforcement learning/RL" → Reinforcement-Learning
# - 如果提到"language model/LLM/MoE/transformer" → LLM
# - 如果提到"deep learning/neural network/CNN/RNN/backpropagation" → Deep-Learning
# - 否则 → Machine-Learning

PAPERS_DIR="${VAULT_ROOT}/20_Research/Papers"
DOMAIN="[LLM/Computer-Vision/Agents/Reinforcement-Learning/Deep-Learning/Machine-Learning]"
...
```bash
# 一键执行
bash run_full_analysis.sh 2602.02276 "Kimi K2.5: Visual Agentic Intelligence" "Kimi Team" "Agents"
```
...
## 批量模式：/paper-analyze all

当用户调用 `/paper-analyze all` 时，自动找到当天或最新的每日推荐笔记，精读其中的所有论文。

### 步骤 All.1：找到最新推荐笔记

```bash
VAULT_ROOT="${OBSIDIAN_VAULT_PATH}"
# 找到 5_Daily/ 下最新的 .md 文件
LATEST_DAILY=$(ls -t "${VAULT_ROOT}/5_Daily/"*.md 2>/dev/null | head -1)

if [ -z "$LATEST_DAILY" ]; then
    echo "未找到每日推荐笔记，请先运行 /start-my-day"
    exit 1
fi

echo "使用推荐笔记: $LATEST_DAILY"
```

### 步骤 All.2：提取所有 arXiv ID

```bash
# 从推荐笔记中提取 arXiv ID，匹配 arxiv.org/abs/XXXX.XXXXX 格式
PAPER_IDS=$(grep -oP 'arxiv\.org/abs/\K\d+\.\d+' "$LATEST_DAILY" | sort -u)
PAPER_COUNT=$(echo "$PAPER_IDS" | grep -c .)

echo "共找到 ${PAPER_COUNT} 篇论文待分析"
echo "$PAPER_IDS"
```

### 步骤 All.3：遍历分析每一篇

**对每个 arXiv ID，必须完整执行以下所有子步骤。禁止跳过任何一步。**

```bash
for PAPER_ID in $PAPER_IDS; do
    echo "=== 开始分析: $PAPER_ID ==="
    
    # 检查已有笔记（按 paper_id 搜索 frontmatter，不是文件名）
    EXISTING=$(grep -rl "paper_id.*${PAPER_ID}" "${VAULT_ROOT}/20_Research/Papers/" 2>/dev/null | head -1)
    
    if [ -n "$EXISTING" ]; then
        echo "已有笔记: $EXISTING"
        # 检查是否已有图片
        NOTE_DIR=$(dirname "$EXISTING")
        IMAGES_DIR="${NOTE_DIR}/$(basename "$EXISTING" .md)/images"
        IMG_COUNT=$(ls "$IMAGES_DIR" 2>/dev/null | grep -c .)
    ...
    # --- All.3.1: 获取论文元数据 ---
    # 使用 WebFetch 访问 arxiv.org/abs/$PAPER_ID
    # 提取：标题、作者、摘要、分类、发布日期
    
    # --- All.3.2: 确定领域 ---
    # 根据分类和摘要判断领域：LLM / Computer-Vision / Deep-Learning / Agents / Reinforcement-Learning / Machine-Learning
    
    # --- All.3.3: 下载源码包并提取图片（必须！禁止跳过）---
    # 运行 extract-paper-images skill 的脚本
    SCRIPT_DIR="${SKILLS_ROOT}/extract-paper-images/scripts"
    PAPER_TITLE_SAFE="<从标题生成的安全文件名>"
    DOMAIN="<推断的领域>"
    
    python3 "${SCRIPT_DIR}/extract_images.py" \
      "$PAPER_ID" \
      "${VAULT_ROOT}/20_Research/Papers/${DOMAIN}/${PAPER_TITLE_SAFE}/images" \
      "${VAULT_ROOT}/20_Research/Papers/${DOMAIN}/${PAPER_TITLE_SAFE}/images/index.md"
    
    # --- All.3.4: 深度分析并生成笔记 ---
    # 按单篇分析流程（步骤 3-4）：
    # - 分析摘要、方法论、实验
    # - 按笔记结构模板生成完整的 markdown 内容
    # - 在笔记中引用提取的图片（![[image_name.png|600]]）
    # - 写入到 ${VAULT_ROOT}/20_Research/Papers/${DOMAIN}/${PAPER_TITLE_SAFE}.md
    
    # --- All.3.5: 更新知识图谱 ---
    python3 "${SKILLS_ROOT}/paper-analyze/scripts/update_graph.py" \
      --paper-id "$PAPER_ID" \
      --title "<论文标题>" \
      --domain "$DOMAIN" \
      --score <评分> \
...
| 论文 | 领域 | 评分 | 状态 |
|------|------|------|------|
| [[笔记1\|标题1]] | LLM | 9.5/10 | 新生成 |
| [[笔记2\|标题2]] | Deep-Learning | 8.0/10 | 新生成 |
| [[笔记3\|标题3]] | LLM | -- | 已存在，跳过 |
```
...
当用户调用 `/paper-analyze [论文ID]` 时（支持单篇 arXiv ID 或 `all`）：
